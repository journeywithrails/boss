...
module Nkryptic # :nodoc:
  module ActsAsConfigurable #:nodoc:
    ...
    module InstanceMethods
      ...
      def reload_with_cached_settings
        @general_settings = nil
        reload_without_cached_settings
      end
      
      def self.included(base) # :nodoc:
        base.extend Nkryptic::ActsAsConfigurable::InstanceMethods::ClassMethods
        base.alias_method_chain :reload, :cached_settings
      end
      ...
      # * Checks self.settings_for(obj)[key] for each obj in chain, returning the first non-nil.
      # Example:
      # tax_enabled = user.chained_setting_for( tax_enabled, invoice, customer )
      # use this for targeted settings.
      def chained_setting_for(key, *chain)
        chain.flatten!
        chain.each do |obj|
          setting = self.settings_for(obj)[key]
      ...
      # Example:
      # tax_enabled = user.chained_setting_for( tax_enabled, invoice, customer )
      def chained_setting(key, *chain)
        chain.flatten!
        chain.each do |obj|
          setting = obj.settings[key]
          return setting unless setting.nil?
        end
        return self.settings[key]
      end
    ...
    class ProxySettings # :nodoc:
      ...
      def []=(name, value) 
        ...
        if setting.is_a? Array
          ...
          setting.save
          p = ProxySetting.new(setting)
        end
        cached_settings[name.to_sym] = setting
        return p
      end
    ...
    class ConfigurableSettings < ProxySettings # :nodoc:
      ...
      def find_settings
        return cached_settings.values if @fetched_cache
        if @reference.is_a? Class
          settings_arr = settings.find( :all, 
        ...
        @cached_settings = Hash[*settings_arr.collect{|v| [v.name, v]}.flatten]
        @fetched_cache = true
        settings_arr
      end
      ...
      def find_setting(name)
        key = name.to_sym
        return cached_settings[key] if cached_settings.has_key?(key)
        if @reference.is_a? Class
          cached_settings[key] = settings.find( :first, 
