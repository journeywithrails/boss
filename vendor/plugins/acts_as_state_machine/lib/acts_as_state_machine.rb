    ...
    module StateMachine              #:nodoc:
      ...
      module ActMacro
        ...
        def acts_as_state_machine(opts)
          ...
          class_inheritable_accessor  :log_events
      ...
      module ClassMethods
        ...
        # RADAR: firing an event uses update_attribute to save the state column, 
        # but update_attribute actually saves all attributes! If refactoring
        # to only update the state column, should probably save if there was
        # an entry action
        def event(event, &block)
          ...
          def #{event.to_s}!
            ...
            next_states.each do |ns|
              if ns.guard(self)
                ...
                did_transition = self.update_attribute(self.class.state_column, ns.to.to_s)
                
                run_transition_action(exitact) if exitact && !loopback
                break
              end
            end
            RAILS_DEFAULT_LOGGER.debug("state_machine ------> \#{self.id} event: #{event.to_s} from: \#{from_state.to_s} to: \#{current_state.to_s} \#{did_transition ? "success" : "fail"}") if self.class.log_events
            did_transition
          end
          EOV
          ...
          if block_given?
            ...
            trannys.each do |tranny|
              froms = (tranny[:from] == :ALL) ? read_inheritable_attribute(:states).keys : Array(tranny[:from])
              froms.each do |s|
