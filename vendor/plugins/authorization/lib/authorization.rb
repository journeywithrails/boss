    ...
    module ControllerInstanceMethods
      ...
      def has_permission?( authorization_expression )
        ...
        if not @options[:allow_guests]
          if @current_user.nil? || @current_user == :false  # We aren't logged in, or an exception has already been raised
            RAILS_DEFAULT_LOGGER.error(".mj. authorization.rb - has_permission?: could not find current_user")
            return false
