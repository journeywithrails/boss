...
module FileColumn # :nodoc:
  ...
  def self.init_options(defaults, model, attr)
    ...
    if (ENV['RAILS_ENV'] == 'test') && !$MAGIC_SUX # temporary hack for the migration to stop using this class altogether
      test_dir = File.join(RAILS_ROOT, "test", "fixtures", "files")
      options[:store_dir] ||= File.join(test_dir, model, attr)
      options[:store_dir].sub!(options[:root_path], test_dir)
    else
  ...
  class BaseUploadedFile # :nodoc:
    ...
    def store_dir
      if options[:store_dir].is_a? Symbol
        ...
        when 'test'
          test_dir = File.join(RAILS_ROOT, "test", "fixtures", "files")
          dir = File.join(test_dir, @instance.send(options[:store_dir]))
        else
    ...
    def store_upload(file)
      ...
      if file.respond_to?(:local_path) and file.local_path and File.exists?(file.local_path)
        ...
        if ENV['RAILS_WINDOWS']        
          ...
          File.open(local_file_path, "w") { |f| f.write(file.read) }
        end
      else
  ...
  class PermanentUploadedFile < RealUploadedFile # :nodoc:
    ...
    def move_from(local_dir, just_uploaded)
      ...
      RAILS_DEFAULT_LOGGER.debug("file_column.rb - 359: @dir: #{@dir}    local_dir: #{local_dir}   just_uploaded: #{just_uploaded}   File.dirname(@dir): #{File.dirname(@dir)}")
      FileUtils.rm_rf @dir
      FileUtils.mkdir_p(File.dirname(@dir)) #hack to handle partitioning
  ...
  def self.generate_temp_name
    now = Time.now
    "#{now.to_i}.#{now.usec}.#{::ProcessId.pid}"
  end
