      ...
      if needs_transform?
        begin
          ...
          if options[:magick][:image_required]
            @magick_errors ||= []
            @magick_errors << "is an invalid image"
          end
          return
        end
...
#        RAILS_DEFAULT_LOGGER.debug("magick_file_column.rb - 45: version: #{version}\noptions[:magick]: #{options[:magick].to_yaml}\nversion_options: #{version_options}")
      else
        version_options = MagickExtension::process_options(version)
#        RAILS_DEFAULT_LOGGER.debug("magick_file_column.rb - 45: version_options: #{version_options}")
      end

      unless File.exists?(absolute_path(version_options[:name]))
        begin
          img = ::Magick::Image::read(absolute_path).first
        rescue ::Magick::ImageMagickError => boom
          # we might be called directly from the view here
          # so we just return nil if we cannot load the image
#          RAILS_DEFAULT_LOGGER.debug("magick_file_column.rb - 57: create_magick_version_if_needed -- err: #{boom} -- returning nil")
          return nil
        end
        dirname = version_options[:name]
        FileUtils.mkdir File.join(@dir, dirname)
...
#      RAILS_DEFAULT_LOGGER.debug("magick_file_column.rb - 64: create_magick_version_if_needed returning #{version_options[:name]}")
