module FormTestHelper
  ...
  class Form
    ...
    def submit_without_clicking_button
      ...
      fields.each do|field|
        puts "field.name: #{field.name}" if $log_on
        params[field.name] = field.value unless field.value.nil? || field.value == [] || params[field.name]
      end # don't submit the nils, empty arrays, and fields already named
      
      # Convert arrays and hashes in param keys, since test processing doesn't do this automatically
      puts "submit_without_clicking_button: params before parsing: #{params.inspect}" if $log_on
      params = ActionController::UrlEncodedPairParser.new(params).result
      puts "submit_without_clicking_button: params after parsing: #{params.inspect}" if $log_on
    ...
    def submit(opts={})
      ...
      puts "formtesthelper.submit -- fields_hash: #{fields_hash.keys.inspect}" if $log_on
    ...
    def fields
      ...
      @fields = fields.group_by {|field_tag| field_tag['name'] }.collect do |name, field_tags|
        puts "doing stuff with #{name}" if $log_on
      ...
      puts "@fields.first: #{@fields.first.class}" if $log_on
      @fields
    end
    ...
    def find_field_by_name(field_name)
      # field_name = field_name.to_s.gsub(/\[\]$/, '') # Strip any trailing empty square brackets  # RADAR: this messes with stuff so I pitched it, but I don't know when pitching it may cause failures
  ...
  class Field
    ...
    def name
      # tag['name'].gsub(/\[\]$/, '') # RADAR: this messes with stuff so I pitched it, but I don't know when pitching it may cause failures
      tag['name']
    end
