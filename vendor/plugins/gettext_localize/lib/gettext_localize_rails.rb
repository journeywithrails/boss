module GettextLocalize
  ...
  module Controller
    ...
    def set_locale_by_param(name='lang')
      ...
      RAILS_DEFAULT_LOGGER.debug("set_locale_by_param locales: #{locales.inspect}")
      return unless locales
      locales.each do |locale|
        if GettextLocalize.has_locale?(locale)
          RAILS_DEFAULT_LOGGER.debug("set_default_locale locale: #{locale.inspect}")
          return set_default_locale(locale)
        else
          RAILS_DEFAULT_LOGGER.debug("GettextLocalize does not have #{locale}")
        end
      end
    end
