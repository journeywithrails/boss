...
ActionController::Base.class_eval do
  ...
  protected
    def append_routing_navigator
      ...
      # yuck!
      return if request.xhr? || options[:off] || (!params[:format].blank? && params[:format] != 'html') || (response.body =~ /<\/body/).nil? ||
      ...
      begin
        @template.base_path = RoutingNavigatorController.view_paths
