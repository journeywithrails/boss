...
filter. For example: 
  ...
  prepend_before_filter CASClient::Frameworks::Rails::Filter, :except => [ :unprotected_action, :another_unprotected_action ]

<b>Once the user has been authenticated, their authenticated username is available under <tt>session[:sage_user]</tt>,</b>
...
By default, the Rails filter will only authenticate with the CAS server when no session[:sage_user] value exists. Once the user 
...
by wiping the local <tt>session[:sage_user]</tt> value periodically to force a CAS re-check.
...
CAS authentication for all actions in a controller except the index action:
  ...
  prepend_before_filter CASClient::Frameworks::Rails::GatewayFilter, :only => :index
  prepend_before_filter CASClient::Frameworks::Rails::Filter, :except => :index
