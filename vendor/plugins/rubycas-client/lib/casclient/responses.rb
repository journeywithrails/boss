module CASClient
  module XmlResponse
    attr_reader :xml, :raw, :parse_datetime
    ...
    def to_yaml_properties
      %w{@raw @parse_datetime @failure_code @failure_message}
    end
  ...
  class ValidationResponse
    ...
    def parse(raw_text)
      ...
      RAILS_DEFAULT_LOGGER.debug raw_text
      RAILS_DEFAULT_LOGGER.debug  "\n------------------------- cas response -------------------------\n\n\n"
      ...
      if is_success?
        ...
        #TODO we should really stick to the cas protocol and use <cas:attributes><cas:attribute><cas:name></cas:name><cas:value></cas:value></cas:attribute></cas:attributes>. when time permits factor here and in cas-server-sage
        @xml.elements.to_a('//cas:authenticationSuccess/*').each do |el|
          # @extra_attributes.merge!(Hash.from_xml(el.to_s)) unless el.prefix == 'cas'
          @extra_attributes[el.name] = el.text.strip unless el.prefix == 'cas'
        end
      elsif is_failure?
    ...
    def to_yaml_properties
      %w{@protocol @user @pgt_iou @proxies @extra_attributes} + super
    end
