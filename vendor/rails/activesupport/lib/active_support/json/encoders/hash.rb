  ...
  def to_json(options = {}) #:nodoc:
    ...
    returning result = '{' do
      result << hash_keys.map do |key|
        ...
        if options[:pass_options] and options[:pass_options][key]
          pass_options = options[:pass_options][key]
        else
          pass_options = options
        end
      
        "#{ActiveSupport::JSON.encode(key)}: #{ActiveSupport::JSON.encode(self[key], pass_options)}"
      end * ', '
      result << '}'
    end
  end
